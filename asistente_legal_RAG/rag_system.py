...
@st.cache_resource # cachea la funcion y la recarga solo si es necesario
def initialize_rag_system():
    ...
    return rag_chain, mmr_multi_retriever, llm_generation, final_retriever, format_docs, prompt


def _format_docs_info(docs):
    """Formatea documentos recuperados para mostrar en la UI."""
    docs_info = []
...
def _calculate_confidence(docs):
    """Calcula un score de confianza basado en los documentos recuperados."""
    if not docs:
        return {"nivel": "bajo", "score": 0, "mensaje": "No se encontraron documentos relevantes"}

    num_docs = len(docs)

    if num_docs >= SEARCH_K:
        return {"nivel": "alto", "score": 100, "mensaje": f"{num_docs} documentos relevantes encontrados"}
    elif num_docs >= 1:
        score = int((num_docs / SEARCH_K) * 100)
        return {"nivel": "medio", "score": score, "mensaje": f"{num_docs} de {SEARCH_K} documentos encontrados"}
    else:
        return {"nivel": "bajo", "score": 0, "mensaje": "No se encontraron documentos relevantes"}
...
def query_rag(question):
    try:
        rag_chain, retriever, *_ = initialize_rag_system()
        ...
        docs_info = _format_docs_info(docs)
        confidence = _calculate_confidence(docs)

        return response, docs_info, confidence

    except Exception as e:
        error_msg = f"Error al procesar la consulta: {str(e)}"
        return error_msg, [], {"nivel": "bajo", "score": 0, "mensaje": "Error en el procesamiento"}


def query_rag_stream(question):
    """Version streaming de query_rag. Retorna (generator, docs_info, confidence)."""
    try:
        _, retriever, llm_generation, final_retriever, format_docs, prompt = initialize_rag_system()
        ...
        context_docs = final_retriever.invoke(question)
        ...
        # Streaming del LLM
        llm_streaming = ChatOpenAI(model=GENERATION_MODEL, temperature=0, streaming=True)

        def token_generator():
            for chunk in llm_streaming.stream(formatted_prompt):
                if chunk.content:
                    yield chunk.content

        return token_generator(), docs_info, confidence

    except Exception as e:
        error_msg = f"Error al procesar la consulta: {str(e)}"
        def error_gen():
            yield error_msg
        return error_gen(), [], {"nivel": "bajo", "score": 0, "mensaje": "Error en el procesamiento"}
