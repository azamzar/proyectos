...
from config import CHROMADB_PATH, DB_PATH, LLM_MODEL, LLM_TEMPERATURE, CONFIDENCE_THRESHOLD
...
Responde solo con "automatico" o "escalado" y una breve justificación (máximo 20 palabras):"""
        ...
        try:
            ...
            if "automatico" in content or "automático" in content:
                ...
                categoria = "automatico" if confianza >= CONFIDENCE_THRESHOLD else "escalado"
            ...
            logger.warning("Rate limit en clasificacion: %s", e)
            categoria = "automatico" if confianza >= CONFIDENCE_THRESHOLD else "escalado"
            return {
                "categoria": categoria,
                "historial": [f"Rate limit alcanzado, clasificando por confianza: {confianza}"]
            }
        except openai.APIError as e:
            logger.error("Error de API en clasificacion: %s", e)
            categoria = "automatico" if confianza >= CONFIDENCE_THRESHOLD else "escalado"
            return {
                "categoria": categoria,
                "historial": [f"Error de API, clasificando por confianza: {confianza}"]
            }
        except Exception as e:
            logger.error("Error inesperado en clasificacion: %s", e, exc_info=True)
            categoria = "automatico" if confianza >= CONFIDENCE_THRESHOLD else "escalado"
            return {
                "categoria": categoria,
                "historial": [f"Error en la clasificacion, usando confianza: {confianza}"]
            }
