...
def configurar_rag():
    ...
    with st.spinner("🔧 Configurando sistema RAG..."):
        # Liberar conexiones de ChromaDB antes de reconstruir
        if "helpdesk" in st.session_state:
            del st.session_state.helpdesk
            gc.collect()
...
def procesar_consulta(consulta: str, ticket_id: str):
    ...
    try:
        ...
        logger.error("Error de API procesando consulta: %s", e)
        st.error(f"Error en el servicio de IA: {str(e)}")
        return None, [], None
    except Exception as e:
        logger.error("Error inesperado procesando consulta: %s", e, exc_info=True)
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.feedback:
            ...
            st.metric("Satisfaccion", f"{positivos}/{total_fb}")
        ...
        with st.form("nueva_consulta"):
            ...
            if submitted:
                ...
                if not consulta.strip():
                    errors.append("La consulta no puede estar vacia.")
                elif len(consulta.strip()) < 10:
                    errors.append("Describe el problema con mas detalle (minimo 10 caracteres).")
                elif len(consulta.strip()) > 2000:
                    errors.append("La consulta es demasiado larga (maximo 2000 caracteres).")
                if usuario.strip() and "@" not in usuario:
                    errors.append("El formato del email no es valido.")
            ...
            for ticket_id, ticket_data in reversed(list(st.session_state.tickets.items())):
                with st.expander(f"🎫 {ticket_id} - {ticket_data['timestamp']}", expanded=True):
                    ...
                    # Respuesta final
                        ...
                        if ticket_id in st.session_state.feedback:
                            fb = st.session_state.feedback[ticket_id]
                            icono = "👍" if fb == "positivo" else "👎"
                            st.info(f"Feedback registrado: {icono}")
                        else:
                            col_fb1, col_fb2 = st.columns(2)
                            with col_fb1:
                                if st.button("👍 Util", key=f"fb_pos_{ticket_id}"):
                                    st.session_state.feedback[ticket_id] = "positivo"
                                    st.rerun()
                            with col_fb2:
                                if st.button("👎 No util", key=f"fb_neg_{ticket_id}"):
                                    st.session_state.feedback[ticket_id] = "negativo"
                                    st.rerun()
