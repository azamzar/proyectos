...
from config import EMBEDDINGS_MODEL, DOCS_PATH, CHROMADB_PATH, CHUNK_SIZE, CHUNK_OVERLAP
...
class DocumentProcessor:
    ...
    def load_documents(self) -> List[Document]:
        ...
        logger.info("Cargando documentos desde %s", self.docs_path)
    ...
    def split_documents(self, documents: List[Document]) -> List[Document]:
        ...
        logger.info("Creados %d chunks", len(chunks))
        return chunks
    
    def create_vectorstore(self, documents: List[Document]) -> Chroma:
        """Crea el vectorstore con ChromaDB."""
        logger.info("Creando vectorstore con ChromaDB...")
        
        # Eliminar coleccion anterior si existe
        if self.chroma_path.exists():
            try:
                ...
                logger.info("Coleccion anterior eliminada")
            except Exception:
                logger.warning("No se pudo eliminar la coleccion anterior")
        ...
        logger.info("Vectorstore creado en %s", self.chroma_path)
        logger.info("Total de vectores: %d", len(documents))
    ...
    def test_search(self, vectorstore: Chroma, query: str = "resetear contraseña"):
        ...
        logger.info("Probando busqueda: '%s'", query)
        
        results = vectorstore.similarity_search(query, k=3)
        
        for i, doc in enumerate(results, 1):
            logger.debug(
                "Resultado %d: tipo=%s, archivo=%s, contenido=%s...",
                i,
                doc.metadata.get('doc_type', 'unknown'),
...
def main():
    ...
    logger.info("Configuracion RAG - Helpdesk 2.0")
    logger.info("=" * 40)
