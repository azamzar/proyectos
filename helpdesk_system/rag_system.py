...
from config import CHROMADB_PATH, EMBEDDINGS_MODEL, LLM_MODEL, LLM_TEMPERATURE_RAG, RETRIEVER_K
...
class VectorRAGSystem:
    ...
    def __init__(self, chroma_path: str = "chroma_db"):
        ...
        self.llm = ChatOpenAI(model=LLM_MODEL, temperature=LLM_TEMPERATURE_RAG)
    ...
    def _load_vectorstore(self):
        ...
        try:
            if not self.chroma_path.exists():
                logger.warning("Vectorstore no encontrado en %s", self.chroma_path)
                return
            ...
            logger.info("VectorRAGSystem inicializado correctamente")
            
        except (ValueError, PermissionError) as e:
            logger.error("Error de configuracion al cargar vectorstore: %s", e)
            self.vectorstore = None
            self.retriever = None
        except Exception as e:
            logger.error("Error inesperado cargando vectorstore: %s", e, exc_info=True)
            self.vectorstore = None
            self.retriever = None
    ...
    def buscar(self, consulta: str) -> Dict[str, Any]:
        ...
        for intento in range(max_reintentos + 1):
            try:
                ...
                # Extraer informacion de los documentos
                contexto_partes = []
                fuentes = []
                ...
                if not contexto_partes:
                    return {
                        "respuesta": "Documentos encontrados pero sin contenido util.",
                        "confianza": 0.2,
                        "fuentes": fuentes
                    }
                ...
                if intento < max_reintentos:
                    espera = 2 ** intento
                    logger.warning("Rate limit alcanzado, reintentando en %ds (intento %d/%d)", espera, intento + 1, max_reintentos)
                    time.sleep(espera)
                else:
                    logger.error("Rate limit alcanzado tras %d reintentos: %s", max_reintentos, e)
                    return {
                        "respuesta": "Servicio temporalmente no disponible. Intente de nuevo en unos minutos.",
                        "confianza": 0.0,
                        "fuentes": []
                    }
            except openai.APIError as e:
                logger.error("Error de API de OpenAI en busqueda RAG: %s", e)
                return {
                    "respuesta": f"Error en el servicio de IA: {str(e)}",
                    "confianza": 0.0,
                    "fuentes": []
                }
            except Exception as e:
                logger.error("Error inesperado en busqueda RAG: %s", e, exc_info=True)
                return {
                    "respuesta": f"Error interno en la busqueda: {str(e)}",
                    "confianza": 0.0,
                    "fuentes": []
                }
...
Respuesta:"""
        ...
        try:
            ...
            logger.error("Error de API generando respuesta: %s", e)
            return f"Error en el servicio de IA: {str(e)}"
        except Exception as e:
            logger.error("Error inesperado generando respuesta: %s", e, exc_info=True)
