...
logger = logging.getLogger("soc.supervisor")
...
TERMINAR después del paso 3. NO continuar."""
    ...
    logger.info("Iniciando supervisor para %s", incident_id)
    logger.info("Flujo: alert_analyzer -> threat_analyzer -> notification_agent")
    ...
    try:
        ...
        logger.info("Supervisor completado para %s | tools=%s", incident_id, tools_used)
        
        return final_result
        
    except Exception as e:
        logger.error("Error en supervisor para %s: %s", incident_id, e)
...
def _extract_agent_result(workflow_result: dict, agent_name: str) -> str:
    try:
        ...
        logger.error("Error extrayendo resultado del agente %s: %s", agent_name, e)
        return ""